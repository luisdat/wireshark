...
sacta_get_gint16(packet_info *pinfo, tvbuff_t *tvb, const gint offset)
{
    ...
    {
        w_size = tvb_get_int16(tvb, offset, ENC_LITTLE_ENDIAN);
    }
    else
    {
        w_size = tvb_get_int16(tvb, offset, ENC_BIG_ENDIAN);
    }
    return w_size;
}
...
sacta_get_gint32(packet_info *pinfo, tvbuff_t *tvb, const gint offset)
{
    ...
    if (isLocalhost(&pinfo->net_dst))
    {
        w_size = tvb_get_int32(tvb, offset, ENC_LITTLE_ENDIAN);
    }
    else
    {
        w_size = tvb_get_int32(tvb, offset, ENC_BIG_ENDIAN);
    }
    return w_size;
}
...
static bool is_sacta_fragmented(tvbuff_t *tvb, packet_info *pinfo)
{
    ...
    if (
        ...
        (end_data == (guint)2 * w_size + TAMANO_CABX ||
         end_data == (guint)2 * w_size + 2 * TAMANO_CABX ||
         end_data == (guint)2 * w_size + 3 * TAMANO_CABX))
    {
        res = false;
    }
...
static bool dissect_sacta_heur_udp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
    ...
    if (tvb_captured_length(tvb) >= SACTA_HEADER_SIZE)
    {
        ...
        else
        {
            ...
            uint8_t sacta_src_dominio = tvb_get_uint8(tvb, 0);
            uint8_t sacta_dst_dominio = tvb_get_uint8(tvb, 4);
            uint8_t sacta_instancia = tvb_get_uint8(tvb, 8);
            uint8_t sacta_sesion = tvb_get_uint8(tvb, 9);
...
int dissect_sacta_nspv(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
    ...
    gint16 tamano_paquete = tvb_get_int16(tvb, offset_sacta_nspv_paquete_len, ENC_BIG_ENDIAN);
    ...
    proto_tree_add_string(sacta_tree, hf_sacta_nspv_paquete, tvb, offset_sacta_nspv_len, tamano_paquete /*tamano_paquete*/, (const char *)paquete_nspv);
...
int dissect_sacta(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
{
    ...
    uint8_t sacta_src_dominio = tvb_get_uint8(tvb, 0);
    uint8_t sacta_src_centro = tvb_get_uint8(tvb, 1);
    uint16_t sacta_src_usuario = sacta_get_gint16(pinfo, tvb, 2);

    // Obtencion del destino
    uint8_t sacta_dst_dominio = tvb_get_uint8(tvb, 4);
    uint8_t sacta_dst_centro = tvb_get_uint8(tvb, 5);
    ...
    else
    {
        ...
        else
        {
            ...
            {
                proto_item *ti_int = proto_tree_add_item(sacta_tree, proto_sacta, tvb, 0, -1, ENC_NA);
                expert_add_info(pinfo, ti_int, &ei_sacta_decompression_failed);
            }
        }
        ...
        if (asterix_handle != NULL && (isAsterix != 0 || isTracks != 0))
        {
            ...
            int16_t asterixLen = tvb_get_int16(data_tvb, 1, ENC_BIG_ENDIAN);
            ...
            if (num_cabx != 0)
            {
                ...
                for (uint32_t i = 0; i < num_cabx; i++, data_offset += TAMANO_CABX, offset += TAMANO_CABX)
                {
                    ...
                    case TIPO_INTERCEPTOR_ARIS:
                        ...
                        proto_tree_add_string(current_cabx_tree, hf_aris_adaptacion, data_tvb, cabx_payload + TAMANO_ARIS_VERSION_ID, TAMANO_ARIS_ADAPTACION, (const char *)adaptacion);
                    ...
                    case TIPO_INTERCEPTOR_EVEREST:
                        ...
                        uint8_t centroOrigen = tvb_get_int8(data_tvb, cabx_payload);
                        ...
                        uint8_t centroDestino = tvb_get_int8(data_tvb, cabx_payload + 1);
